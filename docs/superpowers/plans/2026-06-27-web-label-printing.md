...
## Hardware Verification Results (2026-07-18)

**Verdict: the printing pipeline (Tasks 1–7) is validated on hardware; the Web Serial transport
(Task 5's browser attachment point) is unusable on macOS 26 — not an app bug.**

- A test job generated by the real `brotherDriver` + `rasterCore` code (300 raster lines,
  outlined rectangle, 12mm tape) printed and auto-cut correctly when sent over a direct
  IOBluetooth RFCOMM connection (channel 1). Status replies parsed as expected (error bytes
  8/9, media width at byte 10).
- macOS's virtual Bluetooth serial port (`/dev/cu.PT-P710BT****`) is broken on this machine
...
The `Transport` interface was designed for exactly this swap. Scope sketch for a follow-up
brainstorm/plan session:

- `WebUsbTransport implements Transport` — `navigator.usb.requestDevice` (user gesture, vendor
  filter 0x04F9), claim the printer interface, bulk OUT for job bytes, bulk IN for the 32-byte
  status. Same Brother raster protocol; driver/orchestrator unchanged.
- Profile/UI: `ptP710btProfile` currently binds a serial port; generalize to accept either
  transport (or add a USB profile variant) and decide how the Print button chooses.
- Verified preconditions on this machine: PT-P710BT enumerates over USB (via a monitor hub),
  and the native P-touch app prints from user space — suggesting no kernel driver claims the
  interface (required for WebUSB). To confirm: endpoint layout, and whether the printer needs
  a mode toggle to expose the raster interface over USB.
- Keep the Web Serial transport in the codebase — it is correct per spec and works on OSes with
  functional SPP serial ports; the failure is platform-specific.
